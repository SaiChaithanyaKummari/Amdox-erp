...
import CreateVendorInvoiceModal from '../../components/modals/CreateVendorInvoiceModal.jsx';
...
export default function AccountsPayable() {
  ...
  const fetchInvoices = () => {
    setLoading(true);
    setInvoices([
      { id: 1, invoiceNumber: 'INV-001', vendor: 'Tech Solutions Inc', poNumber: 'PO-001', dueDate: '2024-01-15', amount: 45000, status: 'pending' },
      { id: 2, invoiceNumber: 'INV-002', vendor: 'Office Supplies Co', poNumber: 'PO-002', dueDate: '2024-01-20', amount: 12000, status: 'approved' },
      { id: 3, invoiceNumber: 'INV-003', vendor: 'Industrial Equipment Ltd', poNumber: 'PO-003', dueDate: '2024-01-25', amount: 78000, status: 'pending' },
      { id: 4, invoiceNumber: 'INV-004', vendor: 'Global Logistics', poNumber: 'PO-004', dueDate: '2024-01-30', amount: 23000, status: 'paid' },
      { id: 5, invoiceNumber: 'INV-005', vendor: 'Safety First Supplies', poNumber: 'PO-005', dueDate: '2024-02-05', amount: 15000, status: 'pending' },
    ]);
    setLoading(false);
  };
  ...
  const filteredInvoices = invoices.filter(invoice => {
    const matchesSearch = invoice.invoiceNumber.toLowerCase().includes(searchText.toLowerCase()) ||
                          invoice.vendor.toLowerCase().includes(searchText.toLowerCase()) ||
  ...
  const handleExport = () => {
    exportToCSV(filteredInvoices, 'accounts_payable.csv');
  };
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-xl border border-white/70 bg-white/85 p-6 shadow-soft backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/80">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">Total Outstanding</p>
              <p className="mt-2 text-2xl font-bold text-slate-900 dark:text-slate-100">₹12,45,000</p>
            </div>
        ...
        <div className="rounded-xl border border-white/70 bg-white/85 p-6 shadow-soft backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/80">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">Due This Week</p>
              <p className="mt-2 text-2xl font-bold text-slate-900 dark:text-slate-100">₹1,85,000</p>
            </div>
        ...
        <div className="rounded-xl border border-white/70 bg-white/85 p-6 shadow-soft backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/80">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-2xl font-bold text-slate-900 dark:text-slate-100">₹45,000</p>
            </div>
            <div className="rounded-xl bg-rose-100 p-3 dark:bg-rose-900/30">
              <FiClock className="h-6 w-6 text-rose-600 dark:text-rose-400" />
            </div>
                ...
                {filteredInvoices.length === 0 ? (
                  ...
                  filteredInvoices.map((invoice) => (
                    <tr key={invoice.id} className="border-b border-slate-100 dark:border-slate-800/50 hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                      ...
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{invoice.vendor}</td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{invoice.poNumber}</td>
