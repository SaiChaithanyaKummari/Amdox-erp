...
export default function PurchaseOrders() {
  ...
  const fetchOrders = () => {
    setLoading(true);
    setOrders([
      { id: 1, poNumber: 'PO-001', vendor: 'Tech Solutions Inc', orderDate: '2024-01-10', expectedDate: '2024-01-25', amount: 45000, itemCount: 5, status: 'pending' },
      { id: 2, poNumber: 'PO-002', vendor: 'Office Supplies Co', orderDate: '2024-01-12', expectedDate: '2024-01-28', amount: 12000, itemCount: 2, status: 'approved' },
      { id: 3, poNumber: 'PO-003', vendor: 'Industrial Equipment Ltd', orderDate: '2024-01-15', expectedDate: '2024-02-01', amount: 78000, itemCount: 12, status: 'pending' },
      { id: 4, poNumber: 'PO-004', vendor: 'Global Logistics', orderDate: '2024-01-18', expectedDate: '2024-02-05', amount: 23000, itemCount: 8, status: 'received' },
      { id: 5, poNumber: 'PO-005', vendor: 'Safety First Supplies', orderDate: '2024-01-20', expectedDate: '2024-02-10', amount: 15000, itemCount: 3, status: 'pending' },
    ]);
    setLoading(false);
  };
  ...
  const getStatusBadge = (status) => {
    const styles = {
      ...
      approved: 'bg-orange-100/60 text-orange-600 dark:bg-blue-900/30 dark:text-blue-400',
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <button onClick={handleNewPO} className="erp-focus inline-flex h-11 items-center gap-2 rounded-xl bg-primary px-4 text-sm font-bold text-white shadow-lg shadow-orange-500/20 transition hover:bg-orange-600">
        ...
        <div className="rounded-xl border border-white/70 bg-white/85 p-6 shadow-soft backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/80">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-2xl font-bold text-slate-900 dark:text-slate-100">156</p>
            </div>
        ...
        <div className="rounded-xl border border-white/70 bg-white/85 p-6 shadow-soft backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/80">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-2xl font-bold text-slate-900 dark:text-slate-100">₹45,20,000</p>
            </div>
              ...
              <tbody>
                {filteredOrders.length === 0 ? (
                  ...
                  filteredOrders.map((order) => (
                    <tr key={order.id} className="border-b border-slate-100 dark:border-slate-800/50 hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                      <td className="px-4 py-3 font-medium text-slate-900 dark:text-slate-100">{order.poNumber}</td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{order.vendor}</td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{order.itemCount} items</td>
                      ...
                      <td className="px-4 py-3 text-right font-semibold text-slate-900 dark:text-slate-100">₹{order.amount.toLocaleString()}</td>
