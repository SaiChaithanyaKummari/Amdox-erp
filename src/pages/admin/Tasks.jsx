...
import { FiCheckSquare, FiDownload, FiFilter, FiPlus, FiSearch, FiCheck, FiClock, FiAlertCircle } from 'react-icons/fi';
...
export default function Tasks() {
  ...
  const fetchTasks = () => {
    setLoading(true);
    setTasks([
      { id: 1, code: 'TSK-001', title: 'Design system architecture', project: 'ERP Platform', assignee: 'John Doe', priority: 'high', status: 'in-progress', dueDate: '2024-01-20' },
      { id: 2, code: 'TSK-002', title: 'Implement user authentication', project: 'ERP Platform', assignee: 'Jane Smith', priority: 'high', status: 'completed', dueDate: '2024-01-15' },
      { id: 3, code: 'TSK-003', title: 'Create dashboard widgets', project: 'ERP Platform', assignee: 'Bob Johnson', priority: 'medium', status: 'in-progress', dueDate: '2024-01-25' },
      { id: 4, code: 'TSK-004', title: 'Setup database schema', project: 'ERP Platform', assignee: 'Alice Brown', priority: 'high', status: 'completed', dueDate: '2024-01-10' },
      { id: 5, code: 'TSK-005', title: 'Write API documentation', project: 'ERP Platform', assignee: 'Charlie Wilson', priority: 'low', status: 'pending', dueDate: '2024-02-01' },
    ]);
    setLoading(false);
  };
        ...
        <div className="rounded-xl border border-white/70 bg-white/85 p-6 shadow-soft backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/80">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-2xl font-bold text-slate-900 dark:text-slate-100">212</p>
            </div>
        ...
        <div className="rounded-xl border border-white/70 bg-white/85 p-6 shadow-soft backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/80">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-2xl font-bold text-slate-900 dark:text-slate-100">16</p>
            </div>
                    ...
                    <tr key={task.id} className="border-b border-slate-100 dark:border-slate-800/50 hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                      <td className="px-4 py-3">
                        <div>
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400">{task.code}</p>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{task.project}</td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{task.assignee}</td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{task.dueDate}</td>
                      ...
                      <td className="px-4 py-3 text-center">
                        <button className="erp-focus inline-flex h-8 w-8 items-center justify-center rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-600 shadow-sm transition hover:border-emerald-400 hover:bg-emerald-100 dark:border-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400">
